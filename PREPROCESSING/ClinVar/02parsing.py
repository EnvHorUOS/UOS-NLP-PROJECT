...
out_file = sys.argv[2] # first_preprocessing.tsv
...
################################################################################
# CLNSIG 컬럼의 value_counts() -> txt 파일 제작 (더 자세히 확인하기 위해) / 양성 변이만 파싱
...
# 양성 분류 조건 정의
benign_values = ["Likely_benign", "Benign", "Benign/Likely_benign", "association_not_found"]
# CLNSIG 값이 benign_values에 해당하는 행만 선택
filtered_data = filtered_data[filtered_data['CLNSIG'].isin(benign_values)]
...
# 필요한 경우 파일로 저장
special_references.to_csv('special_references.tsv', sep='\t', index=False)
print("특수 레퍼런스 시퀀스 데이터가 'special_references.tsv'에 저장되었습니다.")
